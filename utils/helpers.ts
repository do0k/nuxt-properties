...
import type { Can, MenuItem, ArrivalCargo, CargoConsumption } from '~/types'
...
export const isRejected = (cargo: ArrivalCargo): boolean => {
  return cargo.confirms.some(c => !c.isConfirmed) && !('confirmed' in (cargo.details || {}))
}
export const hasException = (cargo: ArrivalCargo, as: string): boolean => {
  return cargo.details !== null &&
    'exceptions' in (cargo.details || {}) &&
    Array.isArray(cargo.details?.exceptions) &&
    cargo.details?.exceptions.some(ex => ex.as === as)
}
export const isConfirmed = (cargo:ArrivalCargo):boolean => {
  return 'confirmed' in (cargo.details || {})
}
